...
// next render tick.
func SetLang(l Lang) { activeLang = l }
...
// If the key is missing in the active language the English value is returned.
// If the key is unknown in all languages the key itself is returned so that
// missing translations degrade gracefully rather than panicking.
func T(key string) string {
	if m, ok := translations[activeLang]; ok {
		if s, ok2 := m[key]; ok2 {
			return s
		}
	}
